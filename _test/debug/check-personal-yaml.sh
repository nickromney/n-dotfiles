...
echo "=== Checking _macos/personal.yaml against actual system settings ==="
echo ""
...
# System
echo "=== System ==="
check_setting "Appearance" "$(yq '.system.appearance' "$REPO_ROOT/_macos/personal.yaml")" "$appearance"
check_setting "Show hidden files" "$(yq '.system.show_hidden_files' "$REPO_ROOT/_macos/personal.yaml")" "$hidden_files"
check_setting "Show all extensions" "$(yq '.system.show_all_extensions' "$REPO_ROOT/_macos/personal.yaml")" "$show_extensions"

# Dock
echo ""
echo "=== Dock ==="
check_setting "Position" "$(yq '.dock.position' "$REPO_ROOT/_macos/personal.yaml")" "$dock_position"
check_setting "Auto-hide" "$(yq '.dock.auto_hide' "$REPO_ROOT/_macos/personal.yaml")" "$dock_autohide"
check_setting "Show recents" "$(yq '.dock.show_recents' "$REPO_ROOT/_macos/personal.yaml")" "$show_recents"
check_setting "Animate opening" "$(yq '.dock.animate_opening' "$REPO_ROOT/_macos/personal.yaml")" "$animate_opening"
check_setting "Show indicators" "$(yq '.dock.show_indicators' "$REPO_ROOT/_macos/personal.yaml")" "$show_indicators"
check_setting "Minimize to app" "$(yq '.dock.minimize_to_app' "$REPO_ROOT/_macos/personal.yaml")" "$minimize_to_app"

# Finder
echo ""
echo "=== Finder ==="
check_setting "Show path bar" "$(yq '.finder.show_path_bar' "$REPO_ROOT/_macos/personal.yaml")" "$path_bar"
check_setting "Show status bar" "$(yq '.finder.show_status_bar' "$REPO_ROOT/_macos/personal.yaml")" "$status_bar"

# Keyboard
echo ""
echo "=== Keyboard ==="
check_setting "Key repeat" "$(yq '.keyboard.key_repeat' "$REPO_ROOT/_macos/personal.yaml")" "$key_repeat"
check_setting "Initial key repeat" "$(yq '.keyboard.initial_key_repeat' "$REPO_ROOT/_macos/personal.yaml")" "$initial_repeat"

# Trackpad
echo ""
echo "=== Trackpad ==="
check_setting "Tap to click" "$(yq '.trackpad.tap_to_click' "$REPO_ROOT/_macos/personal.yaml")" "$tap_to_click"
check_setting "Natural scrolling" "$(yq '.trackpad.natural_scrolling' "$REPO_ROOT/_macos/personal.yaml")" "$natural_scroll"
...
check_setting "Auto rearrange" "$(yq '.mission_control.auto_rearrange_spaces' "$REPO_ROOT/_macos/personal.yaml")" "$auto_rearrange"
check_setting "Group by app" "$(yq '.mission_control.group_by_app' "$REPO_ROOT/_macos/personal.yaml")" "$group_by_app"
check_setting "Separate spaces" "$(yq '.mission_control.displays_have_separate_spaces' "$REPO_ROOT/_macos/personal.yaml")" "$separate_spaces"